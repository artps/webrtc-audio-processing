...
namespace webrtc_audio_processing
{
  ...
  Stats get_stats(AudioProcessing *ap)
  {
    ...
    {
      if (p->noise_suppression()->speech_probability() != webrtc::AudioProcessing::kUnsupportedFunctionError)
      {
    ...
    if (p->echo_cancellation()->is_enabled())
    {
      ...
      if (p->echo_cancellation()->GetMetrics(&metrics) == webrtc::AudioProcessing::kNoError)
      {
      ...
      if (p->echo_cancellation()->GetDelayMetrics(
              &delay_median_ms, &delay_stddev_ms, &fraction_poor_delays) == webrtc::AudioProcessing::kNoError)
      {
